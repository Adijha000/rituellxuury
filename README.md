...
- **`SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`** — set both to persist entries in Supabase instead of the local JSON file. Create a `waitlist` table with columns: `id uuid default gen_random_uuid()`, `first_name text`, `email text`, `phone text`, `hair_goal text`, `consent bool`, `source text`, `created_at timestamptz`.
...
The logo (`src/components/Logo.tsx`) is a hand-built SVG recreation of the Rituel leaf mark and wordmark — swap in the brand's original artwork under `public/` and update the component if you have the source file.
